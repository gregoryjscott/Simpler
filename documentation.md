...
Tasks are classes that contain business logic. Tasks are named as verbs instead of nouns, and the name should clearly state the functionality provided by the task. A task class will have optional input properties and/or output properties, a single Execute() method, and possibly sub-tasks properties and that's it. Simpler provides a set of bases classes for defining tasks – Task, InTask, OutTask, and InOutTask.

###Task

The Task class is the base class for all other task classes. It contains an abstract Execute() method, Name property, Stats property, and a static New<TTask>() method that serves as a factory method for instantiating tasks.
...
Fake.SubTasks() will override the behavior of all sub-tasks with an Execute() that does nothing. For tasks with many sub-tasks, it’s easier to start with a call to Fake.SubTasks() and then fake the individual tasks that are necessary for the particular test.
...
GetOne<T>() returns one instance of T by using the given connection and SQL to query the database for a row of data. If values object is provided, it will search the SQL for parameters and use the properties on the values object to create and set the parameter values.
...
GetResult() uses the given connection to execute the given SQL on the database and returns an integer result (usually the number of rows affected). If values object is provided, it will search the SQL for parameters and use the properties on the values object to create and set the parameter values.
...
GetScalar() uses the given connection to execute the given SQL on the database and returns an object result. If values object is provided, it will search the SQL for parameters and use the properties on the values object to create and set the parameter values.
...
Parallel provide syntactic sugar on top of the System.Threading.Parallel for executing tasks in parallel. Parallel.Execute() will accept an variable number of tasks and it will run them in parallel and return after all tasks have been executed.
...
Task.New() appears to just return an instance of the given Task type, however, it actually returns a proxy to the Task. The proxy allows for intercepting Execute() calls to perform actions before and/or after the Task executes. The EventAttribute can be sub-classed and applied to tasks in order to address cross-cutting concerns like logging.
